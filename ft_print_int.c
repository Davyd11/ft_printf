...
/*   Updated: 2020/02/24 16:45:58 by dpuente-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ceros(t_flags *f, int n)
{
	...
	signo = 0;
	if((n < 0) && f->precision == 1) 											//PRECISION 1 IS ONLY WHEN THERE IS A CERO IN THE FLAG	
	{
		write(1,"-",1);
		signo = 1;
	}
	while ((((f->flag_width)-(f->var_width)) > 0) && (f->precision == 1))													//ENTERS ONLY IF THER IS A CERO IN THE FLAG AND A +
	{
		ft_putnbr_fd(0, 1);
		f->flag_width--;
		f->len++;
